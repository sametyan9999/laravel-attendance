...
@php
    ...
    $b1i = $b1 && $b1->break_in_at
        ? Carbon::parse($b1->break_in_at)->format('H:i')
        : '';

    $b1o = $b1 && $b1->break_out_at
        ? Carbon::parse($b1->break_out_at)->format('H:i')
        : '';

    $b2i = $b2 && $b2->break_in_at
        ? Carbon::parse($b2->break_in_at)->format('H:i')
        : '';

    $b2o = $b2 && $b2->break_out_at
        ? Carbon::parse($b2->break_out_at)->format('H:i')
        : '';
        ...
        <form class="dtl-card" method="POST" action="{{ route('attendance.update', $attendance) }}">
            ...
            @foreach($extraBreaks as $index => $br)
                @php
                    $n = $index + 3;
                    $exIn = $br->break_in_at
                        ? Carbon::parse($br->break_in_at)->format('H:i')
                        : '';

                    $exOut = $br->break_out_at
                        ? Carbon::parse($br->break_out_at)->format('H:i')
                        : '';
