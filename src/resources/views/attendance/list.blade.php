...
@section('content')
    ...
    @php
        ...
        $toCarbon = function ($v, $default = null) {
            try {
                ...
                if (is_array($v)) {
                    ...
                    if ($cand instanceof CarbonInterface) {
                        return CarbonImmutable::instance($cand);
                    }

                    if (is_string($cand)) {
                        return CarbonImmutable::parse($cand);
                    }
        ...
        $prevObj = ($prevMonth ?? null)
            ? $toCarbon($prevMonth, $monthObj->subMonth())->startOfMonth()
            : $monthObj->subMonth();

        $nextObj = ($nextMonth ?? null)
            ? $toCarbon($nextMonth, $monthObj->addMonth())->startOfMonth()
            : $monthObj->addMonth();
        ...
        if (!empty($days) && is_iterable($days)) {
            foreach ($days as $d) {
                ...
                if ($c) {
                    $daysList[] = CarbonImmutable::instance($c);
                }
            }
        }
                        ...
                        @forelse ($daysList as $day)
                            ...
                            <tr>
                                ...
                                <td class="col-detail">
                                    @if($id)
                                        <a class="link-detail"
                                           href="{{ route('attendance.detail', ['attendance' => $id]) }}">
                                            詳細
                                        </a>
                                    @else
                                        <span class="link-detail disabled" aria-disabled="true">
                                            詳細
                                        </span>
                                    @endif
                                </td>
